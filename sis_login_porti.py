...
def conexao_banco():
    ...
    con = psycopg2.connect(host='localhost', database='user', user='postgres',password='XXXX' )
    cur = con.cursor()   
...
def entrar():
    ...
    try:
        ...
        v_usuario = cur.fetchall()
        con.close()

            # PRINTA O QUE FOI DIGITADO PELO USUARIO
        print('usuario: '+dig_email.get().capitalize())
        print('senha: '+dig_senha.get())

            # VALIDA O USUARIO SE ESTÁ NO BANCO DE DADOS
        if str(dig_email.get().capitalize()) == str(v_usuario[0][0]):
            print('OK! Logado')
...
def tela_cadastro():
    ...
    button_cadastro.place(y=240, x=160,)
        ...
        #APLICAÇÃO RODANDO NO TKINTER / login
tela = Tk()
